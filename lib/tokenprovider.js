...
TokenProvider.prototype.getToken = function(identity, endpointId) {
  const token = new AccessToken(this.accountSid, this.signingKeySid, this.signingKeySecret, {
    identity: identity,
    ttl: 40000
  });

  const grant = new ChatGrant({ pushCredentialSid: this.pushCredentialSid });
